...
(`-repo-addr` is a URL for the `rest*` repos, `host:port` for the `grpc*` repos.) The comparisons this enables: **HTTP/1.1 vs HTTP/2** (`rest` vs `resth2`), **HTTP/2 vs gRPC** with framing isolated (`resth2-pb` vs `grpc`), and **protobuf vs Avro vs FlatBuffers** on gRPC (`grpc` vs `grpc-avro` vs `grpc-flat`).

Two endpoints sit behind every backend: the single aggregate `/v1/transaction/:id` (~311 B) and the list `/v1/transactions/:count` returning N aggregates. The list exists so the wire codecs can be compared under larger payloads — serialization is rounding error at 311 B and only separates as N grows. `app.TransactionRepository` is `GetByID` + `GetBatch`.

The dataservice exposes all three transports on separate ports (REST/HTTP1 `:9090`, gRPC `:9091`, REST/h2c `:9092`), so a single binary covers every protocol and the front picks at startup. The REST endpoints content-negotiate JSON/protobuf/Avro via the `Accept` header; gRPC serves protobuf, Avro, and FlatBuffers on the same listener via content-subtype. `bench.sh` runs vegeta against each engine in turn and produces a comparative report.
...
cmd/dataservice/grpc.go              #   gRPC service impl, protobuf (GetById + GetBatch)
cmd/dataservice/grpc_avro.go         #   gRPC service impl, Avro (hand-written ServiceDesc)
cmd/dataservice/grpc_flat.go         #   gRPC service impl, FlatBuffers (hand-written ServiceDesc)
proto/transactions.proto             # gRPC wire schema (protobuf): GetById + GetBatch
proto/transactionspb/                # generated protobuf stubs (committed; regen when .proto changes)
proto/transactions.fbs               # FlatBuffers wire schema
proto/transactionsfb/                # generated FlatBuffers code (committed; regen when .fbs changes)
serde/serde.go                       # package serde — Codec port (single+list) + ForAccept factory
serde/json.go,protobuf.go,avro.go    #   JSON / protobuf / Avro codecs; Avro also a grpc encoding.Codec
serde/flatbuffers.go                 #   FlatBuffers builders/readers + grpc encoding.Codec (zero-copy)
...
db/grpcflatclient/grpcflatclient.go  # gRPC client adapter (front → dataservice, FlatBuffers codec)
...
go run main.go -repo grpc-flat   -repo-addr localhost:9091          # gRPC    + FlatBuffers

# Regenerate protobuf stubs (only when proto/transactions.proto changes)
...
# Regenerate FlatBuffers code (only when proto/transactions.fbs changes; needs flatc)
flatc --go -o proto proto/transactions.fbs
...
curl http://localhost:8080/v1/transactions/100   # list of 100 aggregates
...
./bench-full.sh                # 5 × 8 × 2 = 80 cells (~46 min), GetById (~311 B)
./bench-full.sh 10000 60s      # custom rate + duration

# Payload matrix — the GetBatch list endpoint under growing N. Fiber only
# (framework axis settled), 8 backends × 4 sizes. Writes bench-results-payload/.
chmod +x bench-payload.sh
./bench-payload.sh             # fiber × 8 × N{10,100,1000,5000} = 32 cells (~8 min)
./bench-payload.sh 1000 20s    # custom rate + duration (lower rate for big payloads)
...
- **[app/app_test.go](app/app_test.go)** — unit tests for `App.Health` / `App.Hello` / `App.GetTransaction` / `App.GetTransactions`. `fakeTxRepo` is a controllable `TransactionRepository` driving 404 / 500 / 400 / success without a DB. Success cases round-trip the body through `json.Unmarshal` and compare with `reflect.DeepEqual`; the batch test also covers bad `:count` → 400.
...
- **[serde/serde_test.go](serde/serde_test.go)** — round-trip (`Marshal`→`Unmarshal` == original via `reflect.DeepEqual`) for JSON/protobuf/Avro, both single and **list** (`MarshalList`/`UnmarshalList`), plus FlatBuffers single + list and `ForAccept` content-type mapping. UTC micro-aligned timestamps so the Avro/FlatBuffers `timestamp-micros`/int64 dates round-trip exactly.
- **[cmd/dataservice/handler_test.go](cmd/dataservice/handler_test.go)** — handler tests with a `fakeTxRepo`; single 200/404/500 + content negotiation, and the **batch** handler (each `Accept` → matching `Content-Type` + decodable list, plus bad `:count` → 400). No network, no DB.
- **[db/restclient/restclient_test.go](db/restclient/restclient_test.go)** — REST client tests against `httptest.NewServer` stubs. HTTP/1.1: 200/404/5xx/URL-escape. HTTP/2 (`TestGetByID_HTTP2`): real h2c server with content negotiation, asserts `r.ProtoMajor == 2`, round-trips each codec; `GetBatch` round-trips a list over HTTP/2 for each codec.
- **[db/grpcclient/grpcclient_test.go](db/grpcclient/grpcclient_test.go)** — protobuf gRPC client against a real `grpc.Server` (ephemeral port, fake `TransactionServiceServer`). `GetByID` 200/NotFound/Internal + `GetBatch` list round-trip.
- **[db/grpcavroclient/grpcavroclient_test.go](db/grpcavroclient/grpcavroclient_test.go)** — Avro gRPC client against a real `grpc.Server` with a fake `TransactionServiceAvro` (mirrors the hand-written `ServiceDesc`; `HandlerType` must be non-nil — `(*any)(nil)` — or `RegisterService` panics). `GetByID` 200/NotFound/Internal + `GetBatch`, exercising the registered Avro codec over real HTTP/2.
- **[db/grpcflatclient/grpcflatclient_test.go](db/grpcflatclient/grpcflatclient_test.go)** — FlatBuffers gRPC client against a real `grpc.Server` with a fake `TransactionServiceFlat`. `GetByID` 200/NotFound + `GetBatch` list round-trip, exercising the registered flatbuffers codec.
...
- **[serde/](serde/) is where all wire-format coupling lives.** The `Codec` port (`ContentType` + `Marshal`/`Unmarshal` for one aggregate and `MarshalList`/`UnmarshalList` for a batch, all over `app.Transaction`) keeps protobuf/Avro out of the domain — `app.Transaction` stays JSON-tags-only; the protobuf↔domain and Avro↔domain conversions and the Avro/FlatBuffers DTOs live here. `ForAccept` selects a REST codec from an HTTP `Accept` header. Avro and FlatBuffers each register a gRPC `encoding.Codec` (names `avro` / `flatbuffers`) in `init()`, so importing the package is enough to make those gRPC paths available. The protobuf conversions (`TransactionToProto`/`TransactionFromProto`) are shared by the gRPC server, gRPC client, and the protobuf REST codec — no duplication.
- **[serde/flatbuffers.go](serde/flatbuffers.go) is the zero-copy contender.** FlatBuffers is gRPC-only (no REST `Codec`): the outbound message is an already-finished `*flatbuffers.Builder` and the inbound is a generated table Init'd over the received bytes — read-side zero-copy. The schema is [proto/transactions.fbs](proto/transactions.fbs); generated code in [proto/transactionsfb/](proto/transactionsfb/) (committed; regen with `flatc`).
...
- **[db/grpcflatclient/grpcflatclient.go](db/grpcflatclient/grpcflatclient.go) is the FlatBuffers gRPC adapter.** Same shape as the Avro adapter — a hand-written `TransactionServiceFlat` (server in [cmd/dataservice/grpc_flat.go](cmd/dataservice/grpc_flat.go)) selected per-call with `grpc.CallContentSubtype("flatbuffers")`. Both `GetById` and `GetBatch` are implemented; the heavy `GetBatch` path is where zero-copy is meant to pay off.
...
- **[proto/transactions.proto](proto/transactions.proto) is the protobuf gRPC wire schema** (`GetById` + `GetBatch`/`TransactionList`). Mirrors `app.Transaction`. `google.protobuf.Timestamp` for dates; field numbers stable forever. Regen stubs only when it changes. The Avro schema lives as a string in [serde/avro.go](serde/avro.go) (no codegen); the FlatBuffers schema is [proto/transactions.fbs](proto/transactions.fbs) (codegen via `flatc`, dates are unix-micros int64).
...
- **[main.go](main.go) is the composition root** — wiring only, no handler logic. `newServer(engine)` picks the inbound adapter; `newRepo(kind, dsn, repoAddr)` picks the driven adapter (`mysql`/`rest`/`resth2`/`resth2-pb`/`resth2-avro`/`grpc`/`grpc-avro`/`grpc-flat`) and returns a cleanup func; `app.New(txRepo)` injects it into the core. Two routes are registered: `/v1/transaction/:transaction_id` (→ `App.GetTransaction`) and `/v1/transactions/:count` (→ `App.GetTransactions`, the batch). Adding a new HTTP framework = new adapter package + one case in `newServer`. Adding a new transport/serialization = new package implementing the existing port (or a new `serde.Codec` reused by the REST adapter); one case in `newRepo`.
...
- **[bench-full.sh](bench-full.sh)** — *Q2: how does each framework behave under each (transport, serialization)?* Full matrix engine × backend × mode on the **single** aggregate (5 × 8 × 2 = 80 cells, ~46 min). The 8 backends are `mysql`, `rest` (h1+json), `resth2`/`resth2-pb`/`resth2-avro` (h2c + json/pb/avro), `grpc` (pb), `grpc-avro`, `grpc-flat`. Output grouped per engine — 5 sub-tables of 16 rows. Reads directly: HTTP/1.1 vs HTTP/2 (`rest` vs `resth2`), HTTP/2 vs gRPC (`resth2-pb` vs `grpc`), protobuf vs Avro vs FlatBuffers (`grpc` vs `grpc-avro` vs `grpc-flat`). Writes `bench-results-full/`. The `BACKENDS`/`ENGINES` arrays are the trim point if 80 cells is too long.
- **[bench-payload.sh](bench-payload.sh)** — *Q3: how do the wire formats behave as the payload grows?* The **list** endpoint `/v1/transactions/:count`, fiber only (framework axis settled), backend × size: 8 backends × N{10,100,1000,5000} = 32 cells (~8 min, default 500 req/s × 10s — lower because payloads reach ~1.5 MB). Output grouped per N. Writes `bench-results-payload/`. This is where serialization is meant to separate — at ~311 B (bench-full) it's noise.

`bench.sh` and `bench-full.sh` run **two attack modes** back-to-back without restarting the server (bench-payload uses payload size as its second axis instead of modes):
